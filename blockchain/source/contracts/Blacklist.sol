...
contract Blacklist is Ownable {
    ...
    modifier OnlyMarket() {
        require(market != 0x0, "Blacklist.OnlyMarket require market != 0x0");
        require(msg.sender == market || master[msg.sender], "Blacklist.OnlyMarket msg.sender == market || master[msg.sender]");
        _;
    }
    ...
    function Remove(address _whom) public returns (bool) {
        require(blacklisted[msg.sender][_whom] == true, "Blacklist.Remove require blacklisted[msg.sender][_whom] == true");
    ...
    function AddMaster(address _root) public onlyOwner returns (bool) {
        require(master[_root] == false, "Blacklist.AddMaster require master[_root] == false");
        master[_root] = true;
        return true;
    }

    function RemoveMaster(address _root) public onlyOwner returns (bool) {
        require(master[_root] == true, "Blacklist.RemoveMaster require master[_root] == true");
        master[_root] = false;
        return true;
    }
